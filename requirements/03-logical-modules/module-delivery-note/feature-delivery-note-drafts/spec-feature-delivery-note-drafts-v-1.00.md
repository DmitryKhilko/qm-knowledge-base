...
5. Пользователь продолжает заполнять поля формы приватного черновика
...
1. После шага 1 основного потока, пользователь не выполняет ручного сохранения
...
1. На шаге 3 или 7 основного потока возникает ошибка создания/сохранения приватного черновика
...
Выдается сообщение
Когда система успешно сохраняет приватный черновик накладной, система должна отобразить пользователю подтверждающее уведомление «Черновик успешно сохранен».

Ошибка сохранения
Пока пользователь имеет роль BR-DELIVERY-NOTE-DRAFTS-001, если при нажатии кнопки сохранения черновика возникает системная ошибка, то система должна зафиксировать ошибку в лог-файле и уведомить пользователя о сбое.

Выдача сообщения о сбое сохранения
Пока пользователь имеет роль BR-DELIVERY-NOTE-DRAFTS-001 и работает с формой накладной типа BR-DELIVERY-NOTE-DRAFTS-002,
если системе не удается сохранить приватный черновик (как по событию draft_save_interval, так и по нажатию кнопки),
то система должна отобразить уведомление об ошибке «Автосохранение не удалось. Проверьте подключение к сети».
