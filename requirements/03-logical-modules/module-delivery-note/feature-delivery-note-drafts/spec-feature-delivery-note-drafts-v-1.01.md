...
Feature: Создание приватного черновика по требованию пользователя  
    ...
    Scenario: Создание приватного черновика по требованию пользователя (FR)  

        Given пользователь авторизован с ролью [BR-DELIVERY-NOTE-DRAFTS-001][1],  
        And отображается форма накладной типа [BR-DELIVERY-NOTE-DRAFTS-002][2],  
...
Acceptance Criteria:
    ...
    Scenario: Создание уникальной записи в базе данных (AC1)
        Then в таблице базы данных создается новая уникальная запись с типом «Приватный»

    Scenario: Формирование значения полей "Создан" и "Отредактирован" (AC2)
        Then Значения полей "Создан" и "Отредактирован" соответствуют [FR-DELIVERY-NOTE-DRAFTS-CREATE-003](#fr-delivery-note-drafts-create-003)
    
    Scenario: Доступность черновика в реестре (AC3)
        Then созданный черновик доступен для просмотра и выбора в реестре приватных черновиков

    Scenario: Оставаться на форме редактирования (AC4)
